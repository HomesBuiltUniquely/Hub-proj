...
const WardrobeCarousel: React.FC = () => {
  ...
  return (
    <section className="w-full py-12 bg-[#f1f2f6]">
      ...
      <div className="w-[1300px] mx-auto px-4 relative">
        {/* Heading */}
        <div className="mb-6 flex flex-col md:flex-row md:items-end md:justify-between">
          <div>
            <h2 className="text-4xl manrope text-gray-800 mb-1 mt-20">Employees Testimonials</h2>
            <p className="text-gray-500 text-lg">Hear It From Our Hubsters</p>
          </div>
        </div>
        {/* Carousel with arrows overlapping top left */}
        <div className="relative">
          {/* Overlapping Navigation */}
          <div className="absolute -top-20 right-1 mb-2 z-10 flex gap-2 mt-10 mr-10">
            <button
              onClick={() => scroll("left")}
          ...
          >
            {wardrobes.map((item, idx) => (
              <div
                key={idx}
                className="min-w-[290px]  h-[400px] max-w-xs bg-white rounded-3xl shadow-lg group  transition"
              >
                <img
                  src={item.img}
                  alt={item.label}
                  className="w-full h-[200px] object-cover rounded-3xl rounded-b-none  transition duration-300"
                />
