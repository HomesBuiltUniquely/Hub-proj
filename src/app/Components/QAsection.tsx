...
export function QAsection() {
  ...
  return (
    <div className="min-h-screen px-4 sm:px-6 md:px-8 py-20 md:py-16 bg-[#F1F2F6]">
      <div className="max-w-5xl mx-auto">
        <h1 className="text-3xl sm:text-4xl mb-8 sm:mb-10 text-amber-950 font-semibold gilda-display-regular text-center sm:text-left lg:ml-80">
          Frequently Asked Questions
        </h1>

        <div className="space-y-4 sm:space-y-6 mt-15 sm:mt-20">
          {faqData.map((item, index) => (
            <div key={index} className="pb-4 sm:pb-6">
              <div
                className={`border-l-[3px] sm:border-l-[4px] pl-3 sm:pl-4 rounded-sm py-3 sm:py-4 transition-all duration-300 ${
              ...
              >
                ...
                >
                  <span className="text-base sm:text-lg font-medium text-amber-950 pr-4">
                    {item.question}
                  </span>
                  <span className="text-amber-950 flex-shrink-0 ml-2">
                ...
                {openIndex === index && (
                  <div 
                    id={`faq-answer-${index}`}
                    className="mt-3 sm:mt-4 text-amber-950 text-sm sm:text-base leading-relaxed whitespace-pre-line"
                  >
                    {item.answer}
                  </div>
                )}
